...
typedef struct args {
    enum a_mode mode;
    enum egglet_fmt format; /**< Format to archive into. */
    char *egg_file;         /**< Archive file name. */
...
static args chicken_parse_args(int, char **);
...
int main(int argc, char *argv[]) {
    struct args arguments = chicken_parse_args(argc, argv);

    switch (arguments.mode) {
    case A_CHECK: {
        check_egg(arguments.egg_file);
        break;
    }
    case A_LIST: {
        list_egg(arguments.egg_file, false);
        break;
    }
    case A_LIST_LONG: {
        list_egg(arguments.egg_file, true);
        break;
    }
    case A_EXTRACT: {
        extract_egg(arguments.egg_file);
        break;
    }
    case A_CREATE: {
        create_egg(arguments.egg_file, false, arguments.format, arguments.n_paths, arguments.paths);
        break;
    }
    case A_APPEND: {
        create_egg(arguments.egg_file, true, arguments.format, arguments.n_paths, arguments.paths);
        break;
    }
    default: {
        // unreachable
...
#define INVALID_MODE_MESSAGE "Requires exactly one of: 'C|check', 'l|list', 'L|list-long', 'c|create', 'a|append', 'x|extract'"

struct args chicken_parse_args(int argc, char **argv) {
    ...
    while ((opt = getopt_long(
        ...
        case '6': {
            arguments.format = EGGLET_FMT_6;
            break;
        }
        case '7': {
            arguments.format = EGGLET_FMT_7;
            break;
        }
        case '8': {
            arguments.format = EGGLET_FMT_8;
            break;
        }
        case 'C': {
    ...
    if (optind == argc) {
        warnx("Requires exactly one archive (.egg) file");
        warnx("None were given");
        usage_short();
    }

    arguments.egg_file = argv[optind++];
...
static const char *short_usage_message =
    "usage: chicken [<FORMAT>] <MODE> <ARCHIVE-FILE> [<FILE...>]";
...
static const char *long_usage_message =
    "chicken --- a simple file archiver\n"
    "\n"
    "USAGE:\n"
    "    chicken [<FORMAT>] <MODE> <ARCHIVE-FILE> [<FILE...>]\n"
    "\n"
    "COMMON MODES:\n"
    "    -l, --list\n"
